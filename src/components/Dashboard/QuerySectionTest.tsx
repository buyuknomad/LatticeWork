...
const QuerySectionTest: React.FC<QuerySectionTestProps> = ({
  ...
  const safeLimits: QueryLimits = {
    trendingUsed: limits?.trendingUsed || 0,
    trendingLimit: limits?.trendingLimit || 2,
    manualUsed: limits?.manualUsed || 0,
    manualLimit: limits?.manualLimit || 1,
    ...
    <div className="w-full space-y-8 md:space-y-10">
      ...
      {displayTier === 'free' && (
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
            ...
            <div className="bg-[#252525]/50 backdrop-blur-sm rounded-lg p-4 border border-[#333333] hover:border-[#00FFFF]/20 transition-colors">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-[#00FFFF]">
                  {safeLimits.trendingUsed > 0 ? '✓ Used' : '✨ Available'}
                </span>
              </div>
              <div className="text-2xl font-bold text-white">
                {Math.max(0, 1 - Math.min(safeLimits.trendingUsed, 1))}/1
              </div>
            ...
            <div className="bg-[#252525]/50 backdrop-blur-sm rounded-lg p-4 border border-[#333333] hover:border-[#333333]/50 transition-colors">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-400">
                  {safeLimits.trendingUsed > 1 ? '✓ Used' : '📊 Available'}
                </span>
              </div>
              <div className="text-2xl font-bold text-white">
                {Math.max(0, 1 - Math.max(0, safeLimits.trendingUsed - 1))}/1
              </div>
            ...
            <div className="bg-[#252525]/50 backdrop-blur-sm rounded-lg p-4 border border-[#333333] hover:border-[#333333]/50 transition-colors">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-400">
                  {safeLimits.manualUsed > 0 ? '✓ Used' : '✍️ Available'}
                </span>
              </div>
              <div className="text-2xl font-bold text-white">
                {Math.max(0, safeLimits.manualLimit - safeLimits.manualUsed)}/{safeLimits.manualLimit}
              </div>
          ...
          {/* Reset time info */}
          {safeLimits.resetTime && (safeLimits.trendingUsed > 0 || safeLimits.manualUsed > 0) && (
            <p className="text-center text-xs text-gray-500 mt-4">
              Resets in {Math.ceil((safeLimits.resetTime.getTime() - Date.now()) / (1000 * 60 * 60))} hours
            </p>
          )}
        </motion.div>
      )}
      ...
      <SearchSection
        ...
        remainingQueries={displayTier === 'free' ? safeLimits.manualLimit - safeLimits.manualUsed : undefined}
        queryResetTime={safeLimits.resetTime}
        onSubmit={onSubmit}
      ...
      {/* Trending Section - Using safe limits */}
      {trendingQuestions && (
        <TrendingSection
